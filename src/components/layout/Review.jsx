...
export default function Review({ _id, productReview, setProductReview }) {
  ...
  const deleteProductReview = async (reply_id) => {
    try {
      ...
      setProductReview(prevReviews => prevReviews.filter(review => review.id !== reply_id));
    } catch (err) {
      setError(err);
    }
  };
        ...
        <div key={index} className="flex flex-col p-10 border border-solid rounded-lg border-gray1">
          <div className="flex justify-between">
            <div className="flex items-center gap-x-4">
              <img className="w-[50px] h-[50px]" src={"https://11.fesp.shop/files/final06/" + review.user.image} alt="유저 프로필 사진" />
